...
export default {
	...
	props: {
		...
		cols: {
			type: Object,
			default: function() {
				return { lg: 12, md: 9, sm: 6, xs: 3, xxs: 3 };
			},
		},
		preventCollision: {
			type: Boolean,
	...
	mounted: function() {
		...
		this.$emit('layout-mounted', this.calcWidth());
	...
	methods: {
		...
		calcWidth() {
			...
			const breakpoint = getBreakpointFromWidth(this.breakpoints, width);
			const colNum = getColsFromBreakpoint(breakpoint, this.cols);
			return colNum;
		},
